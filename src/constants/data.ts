export const albums = [
	{
		genre: "R&B/Soul",
		height: 200,
		coverImg: "https://media.pitchfork.com/photos/638902d5f777c8e284615da3/1:1/w_1500,h_1500,c_limit/SZA.jpg",
		albums: [
			{
				img: "https://media.pitchfork.com/photos/638902d5f777c8e284615da3/1:1/w_1500,h_1500,c_limit/SZA.jpg",
				name: "SOS",
				artist: "SZA",
				subscription: "Alternative R&B pick",
			},
			{
				img: "https://substackcdn.com/image/fetch/$s_!J-tf!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F6974e7c9-f4b8-43ae-b52f-e940803dec48_1500x1500.jpeg",
				name: "Brandy",
				artist: "Brandy Norwood",
				subscription: "R&B Poping",
			},
			{
				img: "https://www.liveabout.com/thmb/6wZm7iDUWVwVI-JtuMydi5irmIM=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/o-LAURYN-HILL-facebook-56a7d2405f9b58b7d0ee2501.jpg",
				name: "The Miseducation",
				artist: "Lauryn Hill",
				subscription: "Classic R&B",
			},
			{
				img: "https://images.squarespace-cdn.com/content/v1/5f6df5bcd8a0286b9df5948b/1602514046737-OTR3I9PPON7Y2NMY8MUO/472-SZA-Ctrl.jpg",
				name: "Ctrl",
				artist: "SZA",
				subscription: "Alternative R&B pick",
			},
			{
				img: "https://i.redd.it/9f28g4lmso7e1.jpeg",
				name: "Lana",
				artist: "SZA",
				subscription: "Alternative R&B pick",
			},
		],
	},
	{
		genre: "Hip hop",
		height: 250,
		coverImg: "https://i.scdn.co/image/ab67616d0000b273d9985092cd88bffd97653b58",
		albums: [
			{
				img: "https://i.scdn.co/image/ab67616d0000b273d9985092cd88bffd97653b58",
				name: "GNX",
				artist: "Kendrick Lamar",
				subscription: "Popular rap",
			},
			{
				img: "https://www.rollingstone.com/wp-content/uploads/2022/05/Travis-Scott-Astroworld-2018.jpg?w=800",
				name: "Astro world",
				artist: "Travis Scott",
				subscription: "Popular rap",
			},
			{
				img: "https://upload.wikimedia.org/wikipedia/en/5/51/Kendrick_Lamar_-_Damn.png",
				name: "Damn",
				artist: "Kendrick Lamar",
				subscription: "Popular rap",
			},
			{
				img: "https://images.genius.com/1efc5de2af228d2e49d91bd0dac4dc49.300x300x1.jpg",
				name: "Good kid maad city",
				artist: "Kendrick Lamar",
				subscription: "Popular rap",
			},
			{
				img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSXBqhQQu-BZwxbj0BLdRiZVeSweg3giTx4KQ&s",
				name: "Mr. Morale",
				artist: "Kendrick Lamar",
				subscription: "Popular rap",
			},
		],
	},

	{
		genre: "Pop",
		height: 170,
		coverImg:
			"https://upload.wikimedia.org/wikipedia/en/thumb/9/96/Adele_-_25_%28Official_Album_Cover%29.png/250px-Adele_-_25_%28Official_Album_Cover%29.png",
		albums: [
			{
				img: "https://upload.wikimedia.org/wikipedia/en/thumb/9/96/Adele_-_25_%28Official_Album_Cover%29.png/250px-Adele_-_25_%28Official_Album_Cover%29.png",
				name: "Sing my song",
				artist: "SZA",
				subscription: "Classic pop",
			},
		],
	},
	{
		genre: "EDM",
		height: 230,
		coverImg: "https://upload.wikimedia.org/wikipedia/en/thumb/1/14/Inrainbowscover.png/250px-Inrainbowscover.png",
		albums: [
			{
				img: "https://upload.wikimedia.org/wikipedia/en/thumb/1/14/Inrainbowscover.png/250px-Inrainbowscover.png",
				name: "In Rainbows",
				artist: "Radiohead",
				subscription: "Critically acclaimed",
			},
		],
	},
	{
		genre: "Ballad",
		height: 330,
		coverImg:
			"https://upload.wikimedia.org/wikipedia/en/thumb/5/53/Beyonce_-_Lemonade_%28Official_Album_Cover%29.png/250px-Beyonce_-_Lemonade_%28Official_Album_Cover%29.png",
		albums: [
			{
				img: "https://upload.wikimedia.org/wikipedia/en/thumb/5/53/Beyonce_-_Lemonade_%28Official_Album_Cover%29.png/250px-Beyonce_-_Lemonade_%28Official_Album_Cover%29.png",
				name: "Lemonade",
				artist: "Beyoncé",
				subscription: "Visual album",
			},
		],
	},
	{
		genre: "Rock",
		height: 270,
		coverImg:
			"https://upload.wikimedia.org/wikipedia/en/thumb/2/27/Daft_Punk_-_Discovery.png/250px-Daft_Punk_-_Discovery.png",
		albums: [
			{
				img: "https://upload.wikimedia.org/wikipedia/en/thumb/2/27/Daft_Punk_-_Discovery.png/250px-Daft_Punk_-_Discovery.png",
				name: "Daft Punk",
				artist: "Thịnh Suy",
				subscription: "Dance classic",
			},
		],
	},
	{
		genre: "Indie",
		height: 250,
		coverImg: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTUi3naBu_Xwqva4a8es_S-1PnBu--EnR4MvQ&s",
		albums: [
			{
				img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTUi3naBu_Xwqva4a8es_S-1PnBu--EnR4MvQ&s",
				name: "Thanh",
				artist: "Guy-Manuel de Homem-Christo",
				subscription: "Indie All the time",
			},
		],
	},
];

const DATA = [
	{
		id: "1",
		height: 200,
		title: "Nature",
		image: "https://images.unsplash.com/photo-1506744038136-46273834b3fb",
	},
	{
		id: "2",
		height: 100,
		title: "City",
		image: "https://images.unsplash.com/photo-1467269204594-9661b134dd2b",
	},
	{
		id: "3",
		height: 150,
		title: "Beach",
		image: "https://images.unsplash.com/photo-1507525428034-b723cf961d3e",
	},
	{
		id: "4",
		height: 200,
		title: "Mountains",
		image: "https://images.unsplash.com/photo-1501785888041-af3ef285b470",
	},
	{
		id: "5",
		height: 300,
		title: "Forest",
		image: "https://images.unsplash.com/photo-1501785888041-af3ef285b470",
	},
	{
		id: "6",
		height: 200,
		title: "Architecture",
		image: "https://images.unsplash.com/photo-1501594907352-04cda38ebc29",
	},
];
